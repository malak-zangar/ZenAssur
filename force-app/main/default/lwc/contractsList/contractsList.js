...
import getContracts from '@salesforce/apex/AccountController.getContracts';
...
export default class ContractsList extends NavigationMixin(LightningElement) {
    wireResult;
    contracts = [];
    currentPage = 1;
    itemsPerPage = 7;
    ...
    columns = [
        { label: 'Numéro', fieldName: 'ContractNumber',sortable: true },
        { label: 'Date de début', fieldName: 'StartDate',sortable: true},
        { label: 'Date de fin', fieldName: 'EndDate',sortable: true},
    ...
    wiredData(result) {
        ...
        if (data) {
            ...
            this.contracts = this.allContracts; // copie pour affichage
    ...
    handleDeleteRow(recordIdToDelete) {
        if (confirm('Etes-vous sur de vouloir supprimer ce contrat?')) {
            deleteRecord(recordIdToDelete)
                .then(result => {
                    this.showToast('Succès', 'Contrat supprimé avec succès!', 'success', 'dismissable');
                    handleRefreshPage();
 ...
 updatePageData() {
    ...
    this.paginatedContracts = this.contracts.slice(start, end).map(rec => {
        return {
            ...rec,
...
const cloneData = [...this.contracts];
...
if (!searchTerm) {
    ...
    this.contracts = this.allContracts.filter(item =>
        (item.ContractNumber && item.ContractNumber.toString().toLowerCase().includes(searchTerm)) ||
        (item.Opportunity__c && item.Opportunity__c.toLowerCase().includes(searchTerm)) ||
        ...
        (item.ContractNumber && item.ContractNumber.toString().toLowerCase().includes(searchTerm)) ||
        (item.StartDate && item.StartDate.toLowerCase().includes(searchTerm)) ||
        (item.EndDate && item.EndDate.toLowerCase().includes(searchTerm))
    );
}
...
this.updatePageData();
    ...
    handleDownloadCSV() {
        const csvData = this.data.map(contract => ({
        ...
        exportCSV(this.columns, csvData, 'ContractsList');
    }
    ...
    async downloadCSV() {
        ...
        this.downloadCSVFile(csvContent, 'contractsList.csv');
    }
